...
from .models import Randonnee, Forest, Natura
...
def natura_view(request):
    # Logique spécifique pour la page "natura"
    ...
    # Récupérer les randonnées depuis le modèle Randonnée
    naturas = Natura.objects.all()
    naturas_data = list(naturas.values())

    # Ajouter les données de l'API aux données du modèle Randonnee
    combined_data = naturas_data + api_data

    # Convertir les données combinées en JSON
    combined_data_json = json.dumps(combined_data)

    return render(request, 'map/natura.html',  { 'data': combined_data_json})
